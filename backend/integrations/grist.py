...
We don't keep a local copy of template data (see src/settings.py) — this
client is called on demand, with results cached briefly by the caller
(see get_table_cached) to avoid hitting Grist on every request.
"""
...
def get_table_cached(table, force_refresh=False):
    """list_records() with a short cache, keyed per doc + table."""
    cache_key = f"grist:{client.doc_id}:{table}"
    if not force_refresh:
        cached = cache.get(cache_key)
        if cached is not None:
            return cached
    records = client.list_records(table)
    cache.set(cache_key, records, timeout=settings.GRIST_CACHE_TTL_SECONDS)
    return records
